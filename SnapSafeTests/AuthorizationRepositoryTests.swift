...
//    }
    ...
    func test_calculateRemainingBackoffSeconds_exactlyAtExpiry() async {
        ...
        let nowMs = Int64(clock.now.timeIntervalSince1970 * 1000.0)
        let lastFailedMs = nowMs - 4_000 // exactly 4 seconds ago
        ...
        XCTAssertEqual(0, result) // 4 - 4 = 0
    }
    
    func test_calculateRemainingBackoffSeconds_justStartedBackoff() async {
        let failed = 4 // backoff = 2^(4-1) = 8 seconds
        let nowMs = Int64(clock.now.timeIntervalSince1970 * 1000.0)
        let lastFailedMs = nowMs - 500 // 0.5 seconds ago
