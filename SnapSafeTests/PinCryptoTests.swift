...
final class PinCryptoTests: XCTestCase {
    ...
    override func setUp() {
        super.setUp()
        // Use the real hasher (Argon2Kit) via your production PinCrypto
        crypto = PinCrypto() // uses defaults; real Argon2 under the hood
    }
    ...
    func test_verifyPin_returnsTrueForCorrectPIN() throws {
        ...
        let ok = try crypto.verifyPin(pin: pin, stored: hashed, deviceId: deviceId)
    ...
    func test_verifyPin_returnsFalseForIncorrectPIN() throws {
        ...
        let result = try crypto.verifyPin(pin: "5678", stored: hashed, deviceId: deviceId)
    ...
    func test_verifyPin_handlesEmptyPIN() throws {
        ...
        let result = try crypto.verifyPin(pin: "", stored: hashed, deviceId: deviceId)
