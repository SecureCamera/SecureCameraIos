...
class CameraModel: NSObject, ObservableObject {
    ...
    private var lastFocusPoint: CGPoint = .init(x: 0.5, y: 0.5)
    ...
    @objc private func subjectAreaDidChange(notification _: Notification) {
        refocusCamera()
    }
    ...
    func switchLensType(to lensType: CameraLensType) {
        ...
        DispatchQueue.global(qos: .userInteractive).async { [weak self] in
            ...
            if let oldDevice = currentDevice {
                do {
            ...
            do {
                var device: AVCaptureDevice? = switch lensType {
                case .ultraWide:
                    ultraWideDevice
                case .wideAngle:
                    wideAngleDevice
                ...
                if device == nil {
                    if lensType == .ultraWide, wideAngleDevice != nil {
                ...
                {
                    let normalizedGains = normalizeGains(previousGains, for: device)
    ...
    func switchCamera(to position: AVCaptureDevice.Position) {
        ...
        DispatchQueue.global(qos: .userInteractive).async { [weak self] in
            ...
            do {
                ...
                wideAngleDevice = wideAngleCamera(position: position)
